        ...
        private void ReadStream(int firstCharRead, StreamReader streamReader, bool isstdout)
        {
            ...
            lock (this)
            {
                ...
                while (streamReader.Peek() > -1)
                {
                    ...
                    if (ch == '\n')
                    {
                        // Remove any newline characters at the end of our buffer.
                        // TODO: How did they get here?
                        if (streambuffer.Length > 0)
                        {
                            if (streambuffer[streambuffer.Length - 1] == '\n')
                            {
                                streambuffer = streambuffer.Remove(streambuffer.Length - 1, 1);
                            }

                            if (streambuffer[streambuffer.Length - 1] == '\r')
                            {
                                streambuffer = streambuffer.Remove(streambuffer.Length - 1, 1);
                            }
                        }
